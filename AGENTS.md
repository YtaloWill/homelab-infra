...
# Project: homelab (repo-specific, layered on the rules above)

Single-node Proxmox homelab as IaC. Spec-driven (Kiro-style): work flows
requirements → design → tasks → code, all under `.kiro/`.

- **Read first**: `.kiro/steering/{product,tech,structure}.md` (always-true
  context), then the active spec `.kiro/specs/homelab-foundation/`
  (requirements.md, design.md, tasks.md). PLAN.md is the original intent
  capture — the spec supersedes it.
- **Any infra change updates the spec in the same change** (requirements if
  scope moved, design if a decision changed, tasks checkboxes always).
- **Hard constraints**: unprivileged LXC only; no community helper scripts;
  no secrets in the repo (tfvars + ansible-vault, examples committed);
  container rootfs minimal on local-lvm; stateful service config lives on the
  samba share; gluetun stays independent — nothing may depend on it.
